...
COS_PUBLIC_BASE_URL=<可选的 HTTPS CDN 或公有读域名；私有桶留空并由 API 代理读取>
```
...
同一账号可以兼任多个权限，但系统强制禁止提交人复核自己的草稿，并强制至少保留一个启用的所有者。所有非 GET 管理请求均验证安全 Cookie 和 CSRF；登录同时验证 Argon2id 密码与 6 位 TOTP，连续失败会触发锁定。
...
4. 提交后内容冻结。复核者在“草稿与复核”查看发布前/候选字段差异、答题预览和校验结论，批准或驳回。
5. 发布者勾选已批准草稿并发布。系统先生成完整候选快照、计算 SHA-256、上传并回读校验，再在发布锁和单个 MySQL 串行事务中创建不可变版本、切换题目指针和当前发布指针。
...
在线维护学科、章节和首页模块时，必须先在“学科与章节”创建目录变更集。编辑只更新该变更集的完整候选目录，不直接修改线上目录表；提交时内容以 SHA-256 冻结，且提交人不能复核自己的目录。发布者只能选择复核通过且仍基于当前线上目录哈希的变更集，线上目录已变化时必须重新创建、提交和复核。目录变更可单独发布，也可与题目草稿同批发布；在线新建的学科和章节需先发布目录后再创建题目。Excel 批次中的目录候选已随整批题目完成跨人复核，不再重复创建目录变更集，但仍必须整批发布且不能与在线目录候选发生冲突。
