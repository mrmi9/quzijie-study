...
- [ ] 全局错题按七学科及掌握状态筛选准确；切换全部、未掌握、已掌握只改变展示列表，不改变当前学科范围的未掌握数量和重做题池。
- [ ] 全局错题“全部”标签在加载、失败或未掌握数量为 0 时禁用重练；存在未掌握错题时显示并可点击“重做全部未掌握错题（N）”。
- [ ] 答题反馈页收藏、模块收藏页取消和全局收藏数量同步。
- [ ] 全局收藏“全部”标签在加载、失败或空列表时禁用重练；存在收藏时显示并可点击“重练全部收藏”。
- [ ] 全局收藏和全局错题题量页均默认选择“全部”，并可切换 5、10、20；候选数不足固定题量时取现有全部，题目 ID 无重复。
- [ ] 至少准备两个学科的收藏题和未掌握错题后分别统一重练，题目随机混排且每题显示正确学科标签；待答视图不显示正确答案或解析，已掌握错题不会进入题池。
- [ ] 全局收藏和全局错题练习均可使用上一题回顾；退出再进入后恢复到首道未答题，已提交答案、未提交本地草稿和收藏状态均保持一致。
- [ ] 完成后总题数、正确数、错误数和正确率一致；各学科表现按注册顺序显示，章节表现均带正确学科名称。
- [ ] 全局结果页“再练一组”返回相同模式的全局题量页：收藏保持 `scope=all&mode=favorite`，错题保持 `scope=all&mode=wrong`；“查看错题”进入全局错题页，“返回学习首页”进入总首页。
- [ ] 全局错题重做中答对按题目所属学科标记已掌握，答错保持未掌握并累加错误次数；单学科错题和收藏重练入口与原流程保持不变。
...
- [ ] 迁移不会影响其他六学科的初始空状态。
- [ ] 旧 v2 普通练习缺少 `scope` 时按单学科恢复；`scope=all` 的全局收藏或错题会话恢复后学科仍为空，不会误显示为 C/C++。
