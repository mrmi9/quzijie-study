...
- [ ] 确认 `ADMIN_REVIEW_POLICY`：`two-person` 使用非提交人复核；`single-owner` 由 OWNER 完成自检清单、说明和发布前 TOTP 确认。两种模式都不得直接修改生产表。
...
- [ ] 配齐私有 COS、稳定管理密钥和复核策略后以 `ADMIN_ENABLED=true` 再部署，等待 `/ready=200/database ok` 后验证 `/admin/` 和基线快照。
- [ ] 单管理员模式通过 `/admin/setup` 创建首个 OWNER，立即删除 `ADMIN_BOOTSTRAP_TOKEN_HASH`；先发布并回滚小型测试批次，再导入和发布 350 题正式批次。
