...
npm run verify:integration # 测试库迁移与真实 PostgreSQL API 集成测试
npm run verify:all     # 小程序、后端和数据库集成门禁
npm run verify:release # 在全量门禁后检查 HTTPS API、隐私信息和题库人工复核
```
...
真实微信登录、普通练习、408、账户删除和容器化部署基线均已完成。体验版与正式版会强制使用 `miniprogram/config/release.js` 中的 HTTPS API，不接受本地 Storage 覆盖；当前该文件故意留空，必须在预发布域名确定后配置并通过 `npm run verify:release`。正式发布前还需完成 HTTPS 部署、微信合法域名、主体与隐私配置、500 题交叉复核及 iOS/Android 真机验收。在线编程判题、主观题评分、排行榜、评论、搜索和管理后台仍不在首版范围。
