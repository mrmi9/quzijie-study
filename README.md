...
原生微信小程序，当前基线包含 500 道唯一题目、七个底层学科和考研 408 客观题专项模拟；正式题库已升级为 MySQL 驱动的动态目录，可通过中文管理后台扩展到 10 万题。项目保留 Mock 仓储供离线开发演示，正式版始终读取云托管 API。
...
- 题库管理：动态学科/章节、五种题型、Excel 暂存校验、可配置的双人复核/单所有者自检、原子发布、SHA-256 对象快照、回滚、媒体和审计。
...
npm run validate:data  # 校验开发 Mock 快照的动态题量、五种题型、答案、408 题池和图片
...
npm run verify:migration # 从旧结构构造 500 题及历史数据并验证管理系统迁移无损
npm run admin:build    # 构建 /admin 中文题库管理后台
npm run verify:all     # 小程序、后端和数据库集成门禁
npm run verify:release # 在全量门禁后检查云托管配置、隐私信息和题库人工复核
```

正式运行时题库以 MySQL 当前发布版本为准；`miniprogram/data/questions.js` 仅是开发 Mock 快照，不要在分包中复制题库。生产内容通过 `/admin/` 管理，发布后小程序无需重新上传。

## Web 题库管理后台

- 后台入口为同源 `/admin/`，支持电脑侧边栏、平板折叠菜单和手机抽屉导航；手机端的题目、导入、复核、发布与回滚流程均可完成。
- 尚未发布的目录草稿可在“学科与章节”中软作废，普通列表默认隐藏并可通过“显示已作废”追溯；已发布内容仍通过发布记录回滚。
- 首次启用且数据库中没有管理员时，运行 `npm run admin:bootstrap-token --workspace server` 生成一次性原始令牌和 SHA-256，只将输出的 `ADMIN_BOOTSTRAP_TOKEN_HASH` 配置到云环境。访问 `/admin/setup`，依次完成启动令牌校验、TOTP 绑定和所有者账号创建；建号后删除该环境变量。
- `ADMIN_REVIEW_POLICY=two-person` 要求提交人与复核人不同；`single-owner` 允许唯一所有者查看完整差异、确认检查项并填写自检说明后自行批准。任何正在复核的目录、题目或 Excel 批次都可由原提交人撤回修改。
- 发布和回滚必须先预览变更摘要及候选 SHA-256，再逐字确认服务端给出的确认文字并输入当前 TOTP；候选内容变化后旧确认立即失效。
- Excel 导入支持按错误、警告、通过状态分页查看行，并可下载完整 XLSX 校验报告。准备好的 350 题应先抽取小批次发布和真机验证，再提交完整批次，不得绕过校验、自检或发布后验证。

后台生产启用要求私有 COS、稳定的 `ADMIN_ENCRYPTION_KEY`、数据库备份和 `ADMIN_ENABLED=true`。详细环境变量、日常操作与故障恢复见 [标准化题库管理手册](docs/QUESTION_BANK_MANAGEMENT.md) 和 [预发布与生产运行手册](docs/OPERATIONS_RUNBOOK.md)。
...
- [标准化题库管理手册](docs/QUESTION_BANK_MANAGEMENT.md)

## 发布边界

真实微信登录、普通练习、填空/简答、408、积分成就、账户删除、动态题库管理和微信云托管部署基线均已完成。体验版与正式版会强制使用 `miniprogram/config/release.js` 中的云环境和服务配置，不接受本地 Storage 覆盖。正式发布前仍需同步公众平台隐私指引，并完成 iOS/Android 真机验收。在线编程判题、AI 出题/判分、教师批改和评论不在当前范围。
