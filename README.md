...
- 真实普通练习后端：微信登录适配、访问/刷新令牌、七学科统计与章节、练习创建/恢复/判题/结果、错题和收藏。
...
当前真实后端仅完成普通练习开发闭环；生产微信 `code2Session` 仍需正式 AppID/AppSecret，408 服务端流程尚未接入。正式发布前还必须完成 HTTPS 部署、合法域名、主体与隐私配置、500 题交叉复核和真机验收。在线编程判题、主观题评分、排行榜、评论、搜索和管理后台仍不在首版范围。
